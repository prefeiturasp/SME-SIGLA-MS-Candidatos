...
class CandidatoViewSet(viewsets.ModelViewSet):
    ...
    def buscar(self, request: Any) -> Any:
        ...
        cc_list = list(
            ConcursoCandidatoRepository.buscar_por_filtro_candidato(q_obj)
        )
        candidatos_por_uuid = {}
        for cc in cc_list:
            c = cc.candidato
        ...
        for c in candidatos_por_uuid.values():  # type: ignore[assignment]
            item = CandidatoRepository.serializar(c["candidato"])  # type: ignore[index]
            item["concursos"] = ConcursoCandidatoRepository.serializar_lista(
                c["concursos"]  # type: ignore[index]
            )
