...
@patch('candidatos.api.views.habilitados.AgendasApiService.remover_agendas_por_processo_uuid_e_cargo')
def test_habilitados_desconvocar_com_codigo_cargo_chama_agendas_api_service(mock_remover_agendas, api_client, lote):
    ...
    cc = ConcursoCandidato.objects.create(candidato=candidato, lote=lote, codigo_inscricao='1', foi_convocado=True, processo_uuid=processo_uuid, codigo_cargo='1008')
    url = reverse('habilitados-desconvocar')
    resp = api_client.patch(url, {'processo_uuid': str(processo_uuid), 'codigo_cargo': '1008'}, format='json')
    assert resp.status_code == 200
    assert resp.data['total'] == 1
    mock_remover_agendas.assert_called_once_with(processo_uuid=str(processo_uuid), codigo_cargo='1008')
    cc.refresh_from_db()
    assert cc.foi_convocado is False

@patch('candidatos.api.views.habilitados.AgendasApiService.remover_agendas_por_processo_uuid_e_cargo')
def test_habilitados_desconvocar_sem_codigo_cargo_nao_chama_agendas_api_service(mock_remover_agendas, api_client, lote):
    """Verifica desconvocar sem cargo não chama remoção de agendas."""
    processo_uuid = uuid4()
    candidato = criar_candidato('C1', '666.666.666-66')
    ConcursoCandidato.objects.create(candidato=candidato, lote=lote, codigo_inscricao='1', foi_convocado=True, processo_uuid=processo_uuid, codigo_cargo='1008')
    url = reverse('habilitados-desconvocar')
    resp = api_client.patch(url, {'processo_uuid': str(processo_uuid)}, format='json')
    assert resp.status_code == 200
...
@patch('candidatos.api.views.habilitados.AgendasApiService.remover_agendas_por_processo_uuid_e_cargo')
def test_habilitados_desconvocar_erro_agendas_api_mantem_resposta_200(mock_remover_agendas, api_client, lote):
...
def test_habilitados_calculados_sucesso_mockando_externos(mock_gerar_sequencia, mock_buscar_escolhas, api_client):
...
@patch('candidatos.api.views.habilitados.EscolhasService.buscar_escolhas', side_effect=Exception('ms caiu'))
@patch('candidatos.api.views.habilitados.gerar_sequencia_convocados')
def test_habilitados_calculados_quando_escolhas_falha_continua_com_lista_vazia(mock_gerar_sequencia, mock_buscar_escolhas, api_client):
...
class TestReconvocacao:
    ...
    def test_erro_servico_retorna_503(self, api_client):
        ...
        with patch('candidatos.api.views.habilitados.EscolhasService.buscar_reconvocacoes', side_effect=Exception('falha')):
    ...
    def test_sem_reconvocacoes_retorna_lista_vazia(self, api_client):
        ...
        with patch('candidatos.api.views.habilitados.EscolhasService.buscar_reconvocacoes', return_value=[]):
            resp = api_client.get(reverse('habilitados-reconvocacao'), {'concurso_uuid': str(uuid4()), 'quantidade': '1'})
        assert resp.status_code == 200
        assert resp.data == []
    ...
    def test_sem_concurso_uuid_retorna_lista_vazia(self, api_client):
        ...
        with patch('candidatos.api.views.habilitados.EscolhasService.buscar_reconvocacoes', return_value=[{'candidato_uuid': str(uuid4())}]):
    ...
    def test_retorna_convocados_na_lista_reconvocacoes(self, api_client, lote):
        ...
        with patch('candidatos.api.views.habilitados.EscolhasService.buscar_reconvocacoes', return_value=[{'candidato_uuid': str(cc.uuid)}]):
            with patch('candidatos.api.views.habilitados.atualizar_ranking'):
                with patch('candidatos.api.views.habilitados.atualizar_ranking_escolha'):
    ...
    def test_filtra_por_codigo_cargo(self, api_client, lote):
        ...
        with patch('candidatos.api.views.habilitados.EscolhasService.buscar_reconvocacoes', return_value=[{'candidato_uuid': str(cc1.uuid)}, {'candidato_uuid': str(cc2.uuid)}]):
            with patch('candidatos.api.views.habilitados.atualizar_ranking'):
                with patch('candidatos.api.views.habilitados.atualizar_ranking_escolha'):
...
class TestEliminar:
    ...
    def test_sucesso_retorna_200_com_dados(self, api_client, lote):
        ...
        with patch('candidatos.api.views.habilitados.aplicar_eliminacao', return_value=(cc, hist_mock)):
    ...
    def test_value_error_retorna_400(self, api_client):
        ...
        with patch('candidatos.api.views.habilitados.aplicar_eliminacao', side_effect=ValueError('candidato não encontrado')):
