...
def test_buscar_reconvocacoes_nao_200_levanta_request_exception(mock_get):
    ...
    with patch.object(EscolhasService, '_get_base_url', return_value='https://api'), pytest.raises(Exception, match='404'):
...
def test_buscar_reconvocacoes_request_exception_propaga(mock_get):
    ...
    with patch.object(EscolhasService, '_get_base_url', return_value='https://api'), pytest.raises(ConnectionError, match='rede'):
        EscolhasService.buscar_reconvocacoes()
...
def test_buscar_escolhas_nao_200_levanta(mock_get):
    ...
    mock_get.return_value = mock_resp
    with patch.object(EscolhasService, '_get_base_url', return_value='https://api'), pytest.raises(Exception):
...
def test_buscar_escolhas_request_exception_propaga(mock_get):
    ...
    with patch.object(EscolhasService, '_get_base_url', return_value='https://api'), pytest.raises(TimeoutError):
        EscolhasService.buscar_escolhas(concurso_uuid='uu')
...
def test_buscar_reconvocacoes_erro_http_levanta_excecao(settings):
    ...
    with patch('candidatos.service.escolhas_service.http_client.get', return_value=mock_resp), pytest.raises(_requests.RequestException):
        EscolhasService.buscar_reconvocacoes()
...
def test_buscar_reconvocacoes_connection_error_levanta_request_exception(settings):
    ...
    settings.ESCOLHAS_API_URL = 'http://escolhas'
    with patch('candidatos.service.escolhas_service.http_client.get', side_effect=_requests.ConnectionError('timeout')), pytest.raises(_requests.RequestException, match='Erro ao conectar'):
        EscolhasService.buscar_reconvocacoes()
...
def test_buscar_reconvocacoes_timeout_levanta_request_exception(settings):
    ...
    settings.ESCOLHAS_API_URL = 'http://escolhas'
    with patch('candidatos.service.escolhas_service.http_client.get', side_effect=_requests.Timeout('timed out')), pytest.raises(_requests.RequestException):
        EscolhasService.buscar_reconvocacoes()
...
def test_buscar_escolhas_connection_error_levanta_request_exception(settings):
    ...
    settings.ESCOLHAS_API_URL = 'http://escolhas'
    with patch('candidatos.service.escolhas_service.http_client.get', side_effect=_requests.ConnectionError('refused')), pytest.raises(_requests.RequestException, match='Erro ao buscar escolhas'):
        EscolhasService.buscar_escolhas(concurso_uuid='uuid-x')
