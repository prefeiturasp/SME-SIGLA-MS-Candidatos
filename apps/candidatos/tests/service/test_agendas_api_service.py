...
def test_get_base_url_levanta_quando_nao_configurado():
    ...
    with patch('candidatos.service.agendas_api_service.settings') as mock_settings:
...
def test_get_base_url_retorna_url_sem_barra_final():
    ...
    with patch('candidatos.service.agendas_api_service.settings') as mock_settings:
        ...
        assert AgendasApiService._get_base_url() == 'https://agendas.example.com'
...
def test_remover_agendas_200_retorna_json(mock_delete):
    ...
    with patch.object(AgendasApiService, '_get_base_url', return_value='https://agendas'):
        resultado = AgendasApiService.remover_agendas_por_processo_uuid_e_cargo(processo_uuid='proc-uuid', codigo_cargo='cargo-uuid')
...
def test_remover_agendas_envia_delete_com_url_e_params_corretos(mock_delete):
    ...
    with patch.object(AgendasApiService, '_get_base_url', return_value='https://agendas'):
        AgendasApiService.remover_agendas_por_processo_uuid_e_cargo(processo_uuid='proc-uuid', codigo_cargo='cargo-uuid', path='/api/v1/agendas/por-processo-e-cargo/')
    mock_delete.assert_called_once_with('https://agendas/api/v1/agendas/por-processo-e-cargo/', params={'processo_uuid': 'proc-uuid', 'cargo': 'cargo-uuid'}, timeout=AgendasApiService.DEFAULT_TIMEOUT)
...
def test_remover_agendas_nao_200_levanta_erro(mock_delete):
    ...
    with patch.object(AgendasApiService, '_get_base_url', return_value='https://agendas'), pytest.raises(requests.HTTPError, match='500'):
        AgendasApiService.remover_agendas_por_processo_uuid_e_cargo(processo_uuid='proc-uuid', codigo_cargo='cargo-uuid')
...
def test_remover_agendas_request_exception_propaga(mock_delete):
    ...
    with patch.object(AgendasApiService, '_get_base_url', return_value='https://agendas'), pytest.raises(requests.RequestException, match='Erro ao conectar com o microserviço de Agendas'):
        AgendasApiService.remover_agendas_por_processo_uuid_e_cargo(processo_uuid='proc-uuid', codigo_cargo='cargo-uuid')
