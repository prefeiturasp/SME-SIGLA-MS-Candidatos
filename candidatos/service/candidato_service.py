...
def upsert_candidato_e_concurso(data: dict[str, Any]) -> tuple[Candidato, ConcursoCandidato]:
    ...
    concurso = ConcursoCandidato.objects.create(candidato=candidato, codigo_inscricao=data.get('codigo_inscricao', ''), classificacao=_none_if_empty(data.get('classificacao')), pontos=data.get('pontos', ''), classificacao_pcd=classificacao_pcd_val, opcao_concurso=data.get('opcao_concurso', ''), codigo_cargo=data.get('codigo_cargo', ''), cota=data.get('cota', ''), descricao_cargo=data.get('descricao_cargo', ''), df=data.get('df', ''), classificacao_nna=classificacao_nna_val, ano_concurso=data.get('ano_concurso', ''), observacao=data.get('observacao', ''), categoria_efetiva=categoria_efetiva_val)
