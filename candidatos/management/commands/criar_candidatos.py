...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        cidades_estados = [('São Paulo', 'SP'), ('Rio de Janeiro', 'RJ'), ('Belo Horizonte', 'MG'), ('Salvador', 'BA'), ('Brasília', 'DF'), ('Fortaleza', 'CE'), ('Manaus', 'AM'), ('Curitiba', 'PR'), ('Recife', 'PE'), ('Porto Alegre', 'RS')]
        ...
        for i in range(count):
            ...
            candidato = Candidato.objects.create(nome=nomes[i % len(nomes)], cpf=cpf, email=email, telefone=f'({random.randint(11, 99)}) {random.randint(90000, 99999)}-{random.randint(1000, 9999)}', data_nascimento=data_nascimento, genero=random.choice(genero_choices), endereco=f'Rua {random.choice(['das Flores', 'Augusta', 'Paulista', 'Copacabana'])}, {random.randint(100, 999)}', cidade=cidade, estado=estado, cep=f'{random.randint(10000, 99999)}-{random.randint(100, 999)}', status=random.choice(status_choices), observacoes=f'Candidato de exemplo {i + 1}')
            self.stdout.write(f'  ✓ Criado candidato: {candidato.nome} ({candidato.cidade}/{candidato.estado})')
        ...
        self.stdout.write(self.style.SUCCESS(f'📊 Estatísticas: {ativos} ativos, {inativos} inativos, {suspensos} suspensos'))
