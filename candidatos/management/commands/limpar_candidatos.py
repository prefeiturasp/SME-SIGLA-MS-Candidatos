...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not confirm:
            self.stdout.write(
                self.style.WARNING(
                    f'⚠️  Você está prestes a remover {total_registros} candidatos!'
                )
            )
            self.stdout.write(
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'✅ {total_registros} candidatos removidos com sucesso!')
            )
            ...
            if registros_restantes == 0:
                ...
                self.stdout.write(
                    self.style.WARNING(f'⚠️  Ainda restam {registros_restantes} candidatos.')
                )
