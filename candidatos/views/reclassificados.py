...
class ReclassificadosViewSet(viewsets.ViewSet):
    ...
    def list(self, request: Any) -> Any:
        ...
        base = ConcursoCandidato.objects.select_related('candidato', 'lote').filter(lote=lote, eliminado=False, categoria_efetiva='GERAL')
        ...
        data = {'nna': ConcursoCandidatoReclassificadoSerializer(qs_nna, many=True).data, 'pcd': ConcursoCandidatoReclassificadoSerializer(qs_pcd, many=True).data}
