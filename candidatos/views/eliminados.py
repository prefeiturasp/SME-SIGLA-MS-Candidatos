...
class EliminadosViewSet(viewsets.ViewSet):
    ...
    def list(self, request: Any) -> Any:
        ...
        base = ConcursoCandidato.objects.select_related('candidato', 'lote').filter(lote=lote, eliminado=True).filter(classificacao__lte=classificacao_max, classificacao__gte=classificacao_min).filter(historicos_eliminacao__processo_uuid=processo_uuid).distinct()
        ...
        data = {'geral': ConcursoCandidatoEliminadoSerializer(qs_geral, many=True).data, 'nna': ConcursoCandidatoEliminadoSerializer(qs_nna, many=True).data, 'pcd': ConcursoCandidatoEliminadoSerializer(qs_pcd, many=True).data}
