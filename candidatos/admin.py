...
from .models import Candidato, ConcursoCandidato, ConcursoCandidatosLote, Parametrizacao
...
class CandidatoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (('Informações Pessoais', {'fields': ('nome', 'cpf', 'email', 'telefone', 'data_nascimento', 'genero')}), ('Endereço', {'fields': ('endereco', 'cidade', 'estado', 'cep')}), ('Status e Observações', {'fields': ('status', 'observacoes')}), ('Metadados', {'fields': ('criado_em', 'atualizado_em', 'esta_ativo'), 'classes': ('collapse',)}))

@admin.register(ConcursoCandidato)
class ConcursoCandidatoAdmin(admin.ModelAdmin):
    """Define ConcursoCandidatoAdmin."""
    list_display = ['candidato', 'candidato__nome', 'codigo_cargo', 'processo_uuid', 'pontos', 'ranking', 'ranking_escolha', 'classificacao', 'classificacao_pcd', 'classificacao_nna', 'foi_convocado', 'data_convocacao', 'lote__concurso_uuid', 'criado_em']
...
class ParametrizacaoAdmin(admin.ModelAdmin):
    ...
    list_display = ['uuid', 'porcentagem_pcd', 'porcentagem_nna', 'criado_em', 'atualizado_em']
    list_filter = ['criado_em', 'atualizado_em', 'esta_ativo']
    readonly_fields = ['uuid', 'criado_em', 'atualizado_em', 'esta_ativo']
    date_hierarchy = 'criado_em'
    list_per_page = 25
    fieldsets = (('Parâmetros de Cálculo', {'fields': ('porcentagem_pcd', 'porcentagem_nna')}), ('Metadados', {'fields': ('uuid', 'criado_em', 'atualizado_em', 'esta_ativo'), 'classes': ('collapse',)}))
