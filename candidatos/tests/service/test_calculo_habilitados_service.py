...
import pytest
from candidatos.models import Candidato, ConcursoCandidato, ConcursoCandidatoEliminacao, ConcursoCandidatoReclassificacao, ConcursoCandidatosLote
from candidatos.service.calculo_habilitados_service import _atualizar_processo_uuid_para_eliminados, _atualizar_processo_uuid_para_reclassificados, _safe_max_classificacao, calcular_posicao_nna, calcular_posicao_pcd, calcular_quantidade_geral, calcular_quantidade_nna, calcular_quantidade_pcd, gerar_sequencia_convocados
...
def _cc(lote: Any, candidato: Any=None, codigo_cargo: Any='CARGO1', **kwargs: Any) -> Any:
    """Executa  cc.
    ...
    Raises:
        Nenhuma exceção específica documentada.
    """
    return ConcursoCandidato.objects.create(candidato=candidato or _candidato(), lote=lote, codigo_inscricao=kwargs.get('codigo_inscricao', uuid4().hex[:8]), codigo_cargo=codigo_cargo, foi_convocado=kwargs.get('foi_convocado', False), eliminado=kwargs.get('eliminado', False), classificacao=kwargs.get('classificacao'), classificacao_nna=kwargs.get('classificacao_nna'), classificacao_pcd=kwargs.get('classificacao_pcd'), categoria_efetiva=kwargs.get('categoria_efetiva', 'GERAL'))
...
class TestAtualizarProcessoUuidReclassificados:
    ...
    def test_sem_processo_uuid_nao_faz_nada(self, lote: Any) -> None:
        ...
        Raises:
            Nenhuma exceção específica documentada.
        """
        _atualizar_processo_uuid_para_reclassificados(final_itens=[], categoria='NNA', classificacao_attr='classificacao_nna', processo_uuid=None, lote=lote, codigo_cargo='CARGO1')
    ...
    def test_com_limite_atualiza_historicos(self, lote: Any) -> None:
        ...
        """
        c = _candidato()
        cc_menor = ConcursoCandidato.objects.create(candidato=c, lote=lote, codigo_inscricao='r1', codigo_cargo='CARGO1', classificacao_nna=1, classificacao_pcd=None)
        ConcursoCandidatoReclassificacao.objects.create(concurso_candidato=cc_menor, desclassificado_de='NNA', processo_uuid=None)
        obj_maior = MagicMock()
        obj_maior.classificacao_nna = 5
        obj_maior.classificacao_pcd = None
        obj_maior.classificacao = None
        processo_uuid = uuid4()
        _atualizar_processo_uuid_para_reclassificados(final_itens=[obj_maior], categoria='NNA', classificacao_attr='classificacao_nna', processo_uuid=processo_uuid, lote=lote, codigo_cargo='CARGO1')
...
class TestAtualizarProcessoUuidEliminados:
    ...
    def test_sem_processo_uuid_nao_faz_nada(self, lote: Any) -> None:
        ...
        Raises:
            Nenhuma exceção específica documentada.
        """
        _atualizar_processo_uuid_para_eliminados(final_itens=[], processo_uuid=None, lote=lote, codigo_cargo='CARGO1')
    ...
    def test_com_limites_atualiza_historicos(self, lote: Any) -> None:
        ...
        """
        c = _candidato()
        cc = ConcursoCandidato.objects.create(candidato=c, lote=lote, codigo_inscricao='e1', codigo_cargo='CARGO1', eliminado=True, classificacao=3)
        ConcursoCandidatoEliminacao.objects.create(concurso_candidato=cc, processo_uuid=None)
        obj = MagicMock()
        obj.classificacao = 10
        obj.classificacao_nna = None
        obj.classificacao_pcd = None
        processo_uuid = uuid4()
        _atualizar_processo_uuid_para_eliminados(final_itens=[obj], processo_uuid=processo_uuid, lote=lote, codigo_cargo='CARGO1')
...
class TestGerarSequenciaConvocados:
    ...
    def test_sem_candidatos_retorna_lista_vazia(self, lote: Any) -> None:
        ...
        with patch('candidatos.service.calculo_habilitados_service.atualizar_ranking'):
            with patch('candidatos.service.calculo_habilitados_service.atualizar_ranking_escolha'):
                itens, porcentagem_nna, porcentagem_pcd = gerar_sequencia_convocados(5, lote=lote, codigo_cargo='CARGO1')
        assert itens == []
    ...
    def test_filtra_por_escolhas_candidato_uuids(self, lote: Any) -> None:
        ...
        with patch('candidatos.service.calculo_habilitados_service.atualizar_ranking'):
            with patch('candidatos.service.calculo_habilitados_service.atualizar_ranking_escolha'):
                itens, _, _ = gerar_sequencia_convocados(3, lote=lote, codigo_cargo='CARGO1', escolhas_candidato_uuids=[str(cc1.uuid)])
        assert len(itens) == 2
    ...
    def test_com_processo_uuid_chama_atualizacao_historicos(self, lote: Any) -> None:
        ...
        with patch('candidatos.service.calculo_habilitados_service.atualizar_ranking'):
            with patch('candidatos.service.calculo_habilitados_service.atualizar_ranking_escolha'):
                itens, _, _ = gerar_sequencia_convocados(2, lote=lote, codigo_cargo='CARGO1', processo_uuid=processo_uuid)
        assert len(itens) == 2

    def test_marca_promovido_para_geral_quando_geral_tem_classificacao_nna(self, lote: Any) -> None:
        ...
        """
        _cc(lote, codigo_cargo='CARGO1', classificacao=1, classificacao_nna=None, classificacao_pcd=None)
        cc_nna = _cc(lote, codigo_cargo='CARGO1', classificacao=2, classificacao_nna=1, classificacao_pcd=None, categoria_efetiva='GERAL')
        with patch('candidatos.service.calculo_habilitados_service.atualizar_ranking'):
            with patch('candidatos.service.calculo_habilitados_service.atualizar_ranking_escolha'):
    ...
    def test_codigo_cargo_vazio_filtra_candidatos_com_cargo_vazio(self, lote: Any) -> None:
        ...
        Raises:
            Nenhuma exceção específica documentada.
        """
        ConcursoCandidato.objects.create(candidato=_candidato(), lote=lote, codigo_inscricao='vazio', codigo_cargo='', classificacao=1, foi_convocado=False, eliminado=False)
        with patch('candidatos.service.calculo_habilitados_service.atualizar_ranking'):
            with patch('candidatos.service.calculo_habilitados_service.atualizar_ranking_escolha'):
