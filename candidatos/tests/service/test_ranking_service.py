...
def _candidato() -> Any:
    ...
    """
    return Candidato.objects.create(nome='Teste', cpf=f'{uuid4().int % 10 ** 11:011d}', email=f'{uuid4().hex[:8]}@example.com', telefone='', data_nascimento='1990-01-01', genero='M', endereco='', cidade='', estado='', cep='', status='ativo', observacoes='')
...
def dois_cc(lote: Any) -> Any:
    ...
    Raises:
        Nenhuma exceção específica documentada.
    """
    c1 = ConcursoCandidato.objects.create(candidato=_candidato(), lote=lote, codigo_inscricao='1', ranking=0)
    c2 = ConcursoCandidato.objects.create(candidato=_candidato(), lote=lote, codigo_inscricao='2', ranking=0)
    return [c1, c2]
...
def test_atualizar_ranking_atribui_ranking_e_persiste(dois_cc: Any) -> None:
    ...
    Args:
        dois_cc: Parâmetro dois cc da operação.
...
def test_atualizar_ranking_escolha_ordena_pcd_primeiro_e_persiste(lote: Any) -> None:
    ...
    Raises:
        Nenhuma exceção específica documentada.
    """
    c_geral = ConcursoCandidato.objects.create(candidato=_candidato(), lote=lote, codigo_inscricao='g', classificacao=1, classificacao_pcd=None, ranking_escolha=0)
    c_pcd = ConcursoCandidato.objects.create(candidato=_candidato(), lote=lote, codigo_inscricao='p', classificacao=5, classificacao_pcd=1, ranking_escolha=0)
    itens = [c_geral, c_pcd]
