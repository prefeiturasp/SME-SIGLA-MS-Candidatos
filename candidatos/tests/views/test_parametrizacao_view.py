...
def parametrizacao_existente():
    ...
    return Parametrizacao.objects.create(
        porcentagem_pcd=0.05, porcentagem_nna=0.20
    )
...
def parametrizacao_multiplas():
    ...
    param2 = Parametrizacao.objects.create(
        porcentagem_pcd=0.10, porcentagem_nna=0.25
    )
...
def test_list_parametrizacao_returns_most_recent(
    api_client, parametrizacao_multiplas
):
...
def test_retrieve_parametrizacao_when_exists(
    api_client, parametrizacao_existente
):
...
def test_patch_parametrizacao_when_exists(
    ...
    payload = {"porcentagem_pcd": 0.15, "porcentagem_nna": 0.30}
...
def test_patch_parametrizacao_when_not_exists(api_client):
    ...
    payload = {"porcentagem_pcd": 0.10, "porcentagem_nna": 0.25}
    assert Parametrizacao.objects.count() == 0
    response = api_client.patch(url, payload, format="json")
    # ViewSet get_object() retorna queryset.first(); com 0 registros retorna None e o mixin pode criar novo  # noqa: E501
...
def test_patch_parametrizacao_updates_most_recent(
    ...
    payload = {"porcentagem_pcd": 0.20, "porcentagem_nna": 0.35}
